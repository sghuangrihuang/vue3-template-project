...
<template>
  <el-row>
    ...
    <el-col :span="8">
      ...
      <div v-if="tabs_data.queueResult.status == 'waiting'"><strong>当前排队人数 {{ tabs_data.queueResult.waitingCount }}人
        </strong> <br /> <strong>正在处理中，排队过程遇到相同模型会插队处理 </strong><br /><strong>购买点数可以试用更高分辨率和插队生成</strong><br /> 当前模型：{{
          tabs_data.queueResult.sd_models.split('.')[0] }}</div>
    </el-col>
  </el-row>
</template>
